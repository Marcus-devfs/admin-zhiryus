...
const EssayCompose: React.FC = () => {
    const { timeRemaining, setStartEssay, startEssay, userData, essayContent, setCurrentStep, setAlertData, setTimeRemaining, totalTime,
        loading, setLoading
    } = useAppContext();
            ...
            <div className='flex flex-col w-full items-center justify-start gap-6 py-8'>
                <div className={`w-full p-6 bg-white border border-gray-200 rounded-lg shadow container flex-col flex`}>
                    <div className='flex w-full justify-between' onClick={() => setShowDetails({ ...showDetails, instruction: !showDetails?.instruction })}>
                        ...
                        <div>
                            <Image alt='icon-gray' src='/icons/right-arrow.png' width={27} height={15} className={`${showDetails?.instruction && 'rotate-90'}
                ...
                <div className={`w-full p-6 bg-white border border-gray-200 rounded-lg shadow container flex-col flex`}>
                    <div className='flex w-full justify-between' onClick={() => setShowDetails({ ...showDetails, vestibular: !showDetails?.vestibular })}>
                        ...
                        <div>
                            <Image alt='icon-gray' src='/icons/right-arrow.png' width={27} height={15} className={`${showDetails?.vestibular && 'rotate-90'}
                            transition ease-in-out delay-150`} />
                        </div>
                    </div>
                    ...
                    <div className={`${showDetails?.vestibular ? 'flex' : 'hidden'} flex-col py-6 px-3 transition ease-in-out delay-150`}>
                        ...
                        {!startEssay && <div className='w-full flex justify-end items-end'>
                            <Button arrowIcon={!loading} isLoading={loading} text='Começar' onClick={() => handleConfirmStartWriting()} disabled={startEssay} />
                        </div>}
